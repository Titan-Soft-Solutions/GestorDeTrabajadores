...
public class IUAdd {
	...
	private JFrame frmInsertarTrabajador;
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					window.frmInsertarTrabajador.setVisible(true);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	private void initialize() throws Exception {
		//Se carga el fichero
		ges = new Gestion();
		frmInsertarTrabajador = new JFrame();
		frmInsertarTrabajador.setTitle("Añadir Trabajador");
		frmInsertarTrabajador.addWindowListener(new WindowAdapter() {
		});
		frmInsertarTrabajador.setBounds(100, 100, 355, 389);
		frmInsertarTrabajador.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		
		JPanel panel = new JPanel();
		panel.setBackground(new Color(204, 204, 204));
		frmInsertarTrabajador.getContentPane().add(panel, BorderLayout.CENTER);
    ...
    public void Visible(boolean vis){
		frmInsertarTrabajador.setVisible(vis);
	}
}
